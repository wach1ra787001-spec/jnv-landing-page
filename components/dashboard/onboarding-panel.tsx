      ...
      <div className="mx-auto my-2 w-full max-w-2xl rounded-3xl border border-border/60 bg-card/85 shadow-2xl backdrop-blur-2xl sm:my-6">
        ...
        <div className="grid gap-6 p-6 md:grid-cols-[180px_1fr]">
          ...
          <div className="min-h-[250px]">
            ...
            {step === 2 && <div className="space-y-5"><div><h3 className="text-2xl font-semibold">Choose your first playbook</h3><p className="mt-1 text-sm text-muted-foreground">Select an existing strategy or create one using the same playbook builder used in Personal Area.</p></div>{data.playbooks.length > 0 ? <div className="space-y-2"><p className="text-sm font-medium text-foreground">Your existing playbooks</p>{data.playbooks.map((playbook) => <button key={playbook.id} onClick={finish} disabled={busy} className="flex w-full items-center justify-between rounded-xl border border-border/60 p-3 text-left transition hover:border-primary hover:bg-primary/5 disabled:cursor-wait disabled:opacity-60"><span><span className="block font-medium">{playbook.title}</span><span className="text-xs text-muted-foreground">Continue with this playbook</span></span>{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <ChevronRight className="h-4 w-4" />}</button>)}</div> : <p className="rounded-xl border border-dashed border-border/60 p-3 text-sm text-muted-foreground">You do not have any playbooks yet.</p>} {!showPlaybookForm ? <><Button onClick={() => setShowPlaybookForm(true)} className="w-full">Create a new playbook</Button><a href="/dashboard/templates" className="block text-center text-sm text-primary underline-offset-4 hover:underline">Browse templates and playbooks</a></> : <div className="rounded-2xl border border-border/60 bg-background/50 p-2"><CreatePlaybookForm onSubmit={createPlaybook} onCancel={() => setShowPlaybookForm(false)} /></div>}</div>}
