      ...
      <div className="mx-auto my-2 w-full max-w-2xl rounded-3xl border border-border/60 bg-card/85 shadow-2xl backdrop-blur-2xl sm:my-6">
        ...
        <div className="grid gap-6 p-6 md:grid-cols-[180px_1fr]">
          <div className="flex gap-2 md:flex-col">
            {stepLabels.map((label, index) => { const key = index === 0 ? "profile" : index === 1 ? "account" : "playbook"; const missing = data.missingSteps?.includes(key); return <button type="button" key={label} onClick={() => setStep(index)} aria-current={step === index ? "step" : undefined} className={`flex items-center gap-2 rounded-xl px-3 py-2 text-left text-sm transition hover:bg-primary/5 ${missing ? "border border-destructive/40 bg-destructive/10 text-destructive" : step === index ? "bg-primary/10 font-medium text-primary" : "text-muted-foreground"}`}><span className="flex h-6 w-6 items-center justify-center rounded-full border border-current text-xs">{missing ? "!" : step > index ? <Check className="h-4 w-4" /> : index + 1}</span><span className="hidden md:inline">{label}{missing ? " — required" : ""}</span></button> })}
          </div>
