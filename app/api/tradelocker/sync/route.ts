...
export async function POST() {
  ...
  try {
    ...
    }).map((order: any) => { const pnlValue = Number(order.profit ?? order.pnl ?? 0); return { user_id: user.id, account_id: accountRow?.id || null, source: 'tradelocker', symbol: String(order.symbol ?? order.instrument ?? 'UNKNOWN').toUpperCase(), direction: String(order.side ?? '').toUpperCase() === 'SELL' ? 'short' : 'long', entry_price: Number(order.price ?? order.openPrice ?? 0), exit_price: Number(order.closePrice ?? order.price ?? 0) || null, quantity: Number(order.quantity ?? order.volume ?? 0), entry_time: order.openTime ?? order.createTime ?? new Date().toISOString(), exit_time: order.closeTime ?? order.updateTime ?? new Date().toISOString(), pnl: pnlValue, net_pnl: pnlValue, status: 'closed', raw_payload: { ...order, _tradeLockerAccountId: connection.tradelocker_account_id, _tradeLockerServer: connection.tradelocker_server } } })
    if (trades.length) { const { error: insertError } = await supabase.from('csv_imports').upsert(trades.map((trade: any) => ({ ...trade, external_ref: String(trade.raw_payload?.orderId ?? trade.raw_payload?.orderID ?? trade.raw_payload?.id ?? trade.raw_payload?.dealId ?? `${trade.symbol}-${trade.entry_time}`), status: 'closed' })), { onConflict: 'user_id,source,external_ref', ignoreDuplicates: true }); if (insertError) throw insertError }
