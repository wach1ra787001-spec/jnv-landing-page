...
export async function POST() {
  ...
  try {
    ...
    if (connection.token_expires_at && new Date(connection.token_expires_at) <= new Date()) {
      ...
      const refreshResponse = await fetch(`${API_BASE}/auth/jwt/refresh`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ refreshToken }), cache: 'no-store' })
    ...
    const response = await fetch(`${API_BASE}/trade/accounts/${connection.tradelocker_account_id}/ordersHistory`, { headers: { Authorization: `Bearer ${accessToken}`, accNum: String(connection.tradelocker_account_id) }, cache: 'no-store' })
    ...
    const { data: existing } = await supabase.from('trades').select('raw_payload').eq('user_id', user.id).eq('source', 'tradelocker')
    const known = new Set((existing || []).map((trade: any) => String(trade.raw_payload?.orderId ?? trade.raw_payload?.id ?? '')))
    const trades = orders.filter((order: any) => !known.has(String(order.orderId ?? order.id ?? ''))).map((order: any) => ({ user_id: user.id, source: 'tradelocker', symbol: String(order.symbol ?? order.instrument ?? 'UNKNOWN').toUpperCase(), direction: String(order.side ?? '').toUpperCase() === 'SELL' ? 'short' : 'long', entry_price: Number(order.price ?? order.openPrice ?? 0), exit_price: Number(order.closePrice ?? order.price ?? 0) || null, quantity: Number(order.quantity ?? order.volume ?? 0), entry_time: order.openTime ?? order.createTime ?? new Date().toISOString(), exit_time: order.closeTime ?? order.updateTime ?? new Date().toISOString(), pnl: Number(order.profit ?? order.pnl ?? 0), status: 'closed', raw_payload: order }))
